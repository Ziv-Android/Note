...
class WidgetJson(QWidget, Ui_WidgetJson):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
        self.unicodeToChinese = True
        self.base64Decode = True
        # 格式化
        ...
        # Unicode转中文
        self.unicodeToChineseCheckBox.stateChanged.connect(self.json_unicode_to_chinese)
        # Base64
        self.base64valueCheckBox.stateChanged.connect(self.json_base64_decode)
        # 清空
    ...
    def json_format(self):
        self.json_parse(sort_keys=True, indent=4, separators=(', ', ': '), ensure_ascii=self.unicodeToChinese)

    def json_reduce(self):
        self.json_parse(sort_keys=True, ensure_ascii=self.unicodeToChinese)
    ...
    def json_unicode_to_chinese(self, result):
        print("json_unicode_to_chinese", result)
        if result == 2:
            self.unicodeToChinese = False
        else:
            self.unicodeToChinese = True
        self.json_parse(sort_keys=True, indent=4, separators=(', ', ': '), ensure_ascii=self.chineseToUnicode)

    def json_base64_decode(self, result):
        print("json_unicode_to_chinese", result)
        if result == 0:
            self.base64Decode = True
        else:
            self.base64Decode = False
        self.json_parse(sort_keys=True, indent=4, separators=(', ', ': '), ensure_ascii=self.base64Decode)
            ...
            for k, v in data.items():
                ...
                if isinstance(v, (dict, list)):
                    ...
                    if self.base64Decode:
                        v = base64.b64decode(str(v))
                    value = QStandardItem(str(v))
                    parent.appendRow([child, value])
                self.set_tree_view_model_data(child, v)
